...
export function findSyncOffset(
  ...
  if (result.isFound) {
    ...
    // console.log(`[NoiseEstimation DEBUG] noiseRegionStart=${noiseRegionStart}, noiseRegionEnd=${noiseRegionEnd}`);
    // console.log(`[NoiseEstimation DEBUG] totalSamples=${receivedSamples.length}, noiseSampleCount=${noiseSampleCount}`);
    // console.log(`[NoiseEstimation DEBUG] noisePowerSum=${noisePowerSum.toExponential(3)}, estimatedNoisePower=${estimatedNoisePower.toExponential(3)}`);
  } else {
